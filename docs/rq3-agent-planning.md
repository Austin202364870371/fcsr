...
当前 Hard-15 只有 Plan-and-Execute 的 planner 半段，尚未接入环境执行。RQ3 研究显式状态和 verifier 驱动重规划，能否比一次性计划或纯 ReAct 循环提高复杂任务成功率并控制成本。
...
```text
retrieve/organize -> plan -> select skill -> act -> observe -> verify
                                                |              |
                                                +---- fail -----+ -> replan
```

状态记录任务 ID、候选 Skill ID、组织版本、计划版本、当前子任务、动作参数、观察、verifier 结果、失败类别、步骤/token/时间预算。重规划必须保留旧计划和触发事件，不得覆盖 trace。
...
三组共享模型、任务、候选、组织、工具、步骤和 token 上限。先固定 Flat 比较规划器，再将最佳机制交叉验证到 RQ2 的三种组织结构。
...
RQ3 不改变 RQ1 检索池或 RQ2 组织定义。文字反思与经验记忆由 RQ4 负责；RQ3 只处理基于执行状态的计划和重规划。
